...
vi.mock('../../hooks/useAuth', () => ({
  useAuth: () => ({ user: { firstName: 'Ana', lastName: 'Silva' }, logout: vi.fn() }),
}))
...
describe('MyHistoryPage', () => {
  ...
  it('shows Confirmar execução button for AwaitingCompletion sessions', async () => {
    ...
    // Session 2 has canConfirm: true
    expect(screen.getByTestId('confirm-btn-2')).toBeInTheDocument()
    // Session 1 does not
    expect(screen.queryByTestId('confirm-btn-1')).not.toBeInTheDocument()
  })
  ...
  it('opens confirm execution modal from the confirm button', async () => {
    ...
    fireEvent.click(screen.getByTestId('confirm-btn-2'))
    await waitFor(() => {
